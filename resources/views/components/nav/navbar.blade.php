<nav class="bg-gray-800">
        ...
        <div class="flex h-16 items-center justify-between">
            <div class="flex items-center">
                ...
                <div class="hidden md:block ml-10 flex items-baseline space-x-4">
                    <x-nav-link href="{{ route('signup.index') }}" :active="request()->routeIs('signup.index')">
                        Sign up
                    </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': !open}" class="md:hidden" id="mobile-menu">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            <x-nav-link href="{{ route('signup.index') }}" :active="request()->routeIs('signup.index')" mobile>
                Sign up
            </x-nav-link>
            <x-nav-link href="{{ route('game.index') }}" :active="request()->routeIs('game.index')" mobile>
                Game
            </x-nav-link>
        </div>
    </div>
